/*
    Example Name: Example2 For SimpleVector Library
...
void setup() {
    ...
    SimpleVector<String> numbers;  // Create a SimpleVector of integers 

    // Add some numbers to the vector
    for(int i = 0; i < 10; i++) {
        numbers.put(String(i)); // Add an element to the vector with value i
    }
    ...
    SimpleVector<String>::SimpleVectorIterator iter = numbers.begin(); // Create an iterator for the vector
    Serial.println("Elements using iterator:");
    while (iter.hasNext()) { // Check if there are more elements in the vector
        String value = iter.next(); // Get the next element from the vector
