...
bool EthernetMach2ConnectionChecker::uploadSnapshotsToUnoQ(UnoQFileTransferClient& client, const String& basePath) {
    ...
    bool okDiag = client.writeText(root + "/diagnostics.json", diagnosticsJson(), false);
    bool okResults = client.writeText(root + "/results.json", resultsJson(), false);
    bool okReg = client.writeText(root + "/registration.json", registrationJson(), false);
    bool okHeartbeat = client.writeText(root + "/heartbeat.json", heartbeatJson(), false);
    return okDiag && okResults && okReg && okHeartbeat;
}

bool EthernetMach2ConnectionChecker::saveSnapshotsToFilesystem(fs::FS& filesystem, const String& basePath) {
    String root = basePath;
    if (root.length() == 0) {
        root = "/em2";
    } else if (!root.startsWith("/")) {
    ...
    File diag = filesystem.open((root + "/diagnostics.json").c_str(), "w");
    if (!diag) return false;
    diag.print(diagnosticsJson());
    diag.close();

    File results = filesystem.open((root + "/results.json").c_str(), "w");
    if (!results) return false;
    results.print(resultsJson());
    results.close();

    File reg = filesystem.open((root + "/registration.json").c_str(), "w");
    if (!reg) return false;
    reg.print(registrationJson());
    reg.close();

    File hb = filesystem.open((root + "/heartbeat.json").c_str(), "w");
    if (!hb) return false;
    hb.print(heartbeatJson());
    hb.close();
