...
JSON::JSON() : _filesystem(nullptr) {
    resetRoot(ValueType::Object);
    sdInstance.begin(SD_CS_PIN);
}

JSON::JSON(size_t cs_pin) : _filesystem(nullptr) {
    resetRoot(ValueType::Object);
    sdInstance.begin(static_cast<uint8_t>(cs_pin));
}

JSON::JSON(fs::FS& filesystem) : _filesystem(&filesystem) {
    resetRoot(ValueType::Object);
}

JSON::~JSON() {}

bool JSON::beginSD(size_t cs_pin) {
    _filesystem = nullptr;
    return sdInstance.begin(static_cast<uint8_t>(cs_pin));
}

bool JSON::begin(fs::FS& filesystem) {
...
int JSON::readFromFile(const char* filename) {
    ...
    if (_filesystem != nullptr) {
        ...
        char chunk[256];
        int bytesRead = 0;
        while ((bytesRead = file.read(reinterpret_cast<uint8_t*>(chunk), sizeof(chunk))) > 0) {
...
int JSON::writeToFile(const char* filename, bool pretty) {
    ...
    if (_filesystem != nullptr) {
        ...
        wroteHeader = file.write(header, sizeof(header)) == sizeof(header);
        wrotePayload = file.write(reinterpret_cast<const uint8_t*>(compressedBuffer), static_cast<size_t>(compressedSize)) == static_cast<size_t>(compressedSize);
        file.close();
    } else {
        ...
        wroteHeader = file.write(header, sizeof(header)) == sizeof(header);
        wrotePayload = file.write(compressedBuffer, static_cast<size_t>(compressedSize)) == static_cast<size_t>(compressedSize);
        file.close();
    }
...
int JSON::readFromUnoQ(UnoQFileTransferClient& client, const String& remotePath) {
    String content;
    if (!client.readText(remotePath, content)) {
        return JSON_READ_ERROR;
    }
    return readFromString(content) ? JSON_READ_SUCCESS : JSON_FILE_PARSE_ERROR;
}

int JSON::writeToUnoQ(UnoQFileTransferClient& client, const String& remotePath, bool pretty) {
    char* serialized = writeToString(pretty);
    if (!serialized) {
        return JSON_WRITE_ERROR;
    }

    String payload(serialized);
    std::free(serialized);

    if (!client.writeText(remotePath, payload, false)) {
        return JSON_WRITE_ERROR;
    }
