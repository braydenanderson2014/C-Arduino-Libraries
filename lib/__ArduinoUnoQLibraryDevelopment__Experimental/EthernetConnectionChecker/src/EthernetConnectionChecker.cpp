...
bool EthernetConnectionChecker::beginFilesystem(fs::FS& filesystem) {
...
bool EthernetConnectionChecker::saveResultCsv(const ECCProbeResult& result) {
    ...
    File file;
    if (_filesystem != nullptr) {
        file = _filesystem->open("/netlog.csv", "a");
    } else {
        selectSdBus();
        file = SD.open("/netlog.csv", FILE_WRITE);
    }
    if (!file) {
        if (_filesystem == nullptr) {
            selectEthernetBus();
...
bool EthernetConnectionChecker::saveResultBinary(const ECCProbeResult& result) {
    ...
    File file;
    if (_filesystem != nullptr) {
        file = _filesystem->open("/netmetric.bin", "a");
    } else {
        selectSdBus();
        file = SD.open("/netmetric.bin", FILE_WRITE);
    }
    if (!file) {
        if (_filesystem == nullptr) {
            selectEthernetBus();
...
bool EthernetConnectionChecker::saveResultText(const ECCProbeResult& result) {
    ...
    File file;
    if (_filesystem != nullptr) {
        file = _filesystem->open("/payloads.txt", "a");
    } else {
        selectSdBus();
        file = SD.open("/payloads.txt", FILE_WRITE);
    }
    if (!file) {
        if (_filesystem == nullptr) {
            selectEthernetBus();
...
bool EthernetConnectionChecker::uploadSnapshotsToUnoQ(UnoQFileTransferClient& client, const String& basePath) {
    ...
    bool okSpeed = client.writeText(root + "/speed.json", speedJson(), false);
    return okDiag && okResults && okSpeed;
}

String EthernetConnectionChecker::normalizePath(const String& path) const {
    if (path.length() == 0) {
        return "/";
    }
    if (path.startsWith("/")) {
