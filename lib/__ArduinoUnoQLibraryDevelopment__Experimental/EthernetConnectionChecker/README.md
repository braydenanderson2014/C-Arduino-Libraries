...
- Library role: Ethernet diagnostics with SD/FS logging and UnoQ snapshot upload

## UnoQ Experimental Status

- This copy is maintained in the UnoQ experimental folder.
- Added filesystem injection support:
  - `beginFilesystem(fs::FS&)`
- Added UnoQ backend snapshot export:
  - `uploadSnapshotsToUnoQ(UnoQFileTransferClient&, const String& basePath)`
- Existing SD logging remains supported (`/netlog.csv`, `/netmetric.bin`, `/payloads.txt`).
