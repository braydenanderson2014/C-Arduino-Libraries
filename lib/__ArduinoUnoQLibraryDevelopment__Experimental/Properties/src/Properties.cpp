...
bool Properties::saveToUnoQ(UnoQFileTransferClient& client, const String& remotePath) {
    String payload;
    char sep = getSeparator();
    for (PropertiesIterator it = begin(); it != end(); ++it) {
    ...
    return client.writeText(remotePath, payload, false);
}

bool Properties::loadFromUnoQ(UnoQFileTransferClient& client, const String& remotePath) {
    String payload;
    if (!client.readText(remotePath, payload)) {
        return false;
    }
    ...
    while (start <= static_cast<int>(payload.length())) {
        ...
        if (end >= static_cast<int>(payload.length())) {
            break;
        }
        start = end + 1;
    }
