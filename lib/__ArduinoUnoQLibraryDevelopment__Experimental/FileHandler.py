...
- write_text(path, content, encoding="utf-8", append=False, create_dirs=True)
- read_bytes(path, offset=0, size=None) -> base64 data
- write_bytes(path, data(base64), append=False, create_dirs=True)
- delete(path, recursive=False)
- rename(src, dst, replace=False)
- copy(src, dst, replace=False)

All paths are sandboxed inside --root.
"""
...
class FileBackend:
	...
	def listdir(self, path: str = ".", recursive: bool = False) -> Dict[str, Any]:
		...
		entries = []
		if recursive:
			iterator: Iterable[Path] = base.rglob("*")
		else:
...
def dispatch(backend: FileBackend, request: Dict[str, Any]) -> Dict[str, Any]:
	...
	try:
		...
		return _error_response(request_id, "INVALID_BASE64", "data is not valid base64")
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		help="Sandbox root directory for all file operations",
	)
	parser.add_argument(
		"--mode",
	...
	parser.add_argument(
		"--once",
		default="",
		help="Process one JSON request and exit (Yun/process bridge mode)",
	)
